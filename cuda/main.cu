...
__global__ void blurKernel(float *outputData, int width, int height, int filterSize){
    ...
    if(x <= pixelRadius || y <= pixelRadius || x >= (width-pixelRadius) || y >= (height-pixelRadius)){
        ...
        //         if(window[(tid * arraySize) + (i * filterSize) + j] < window[(tid * arraySize) + (i * filterSize) + min]){
        //             min = j;
        //         }
        //     }
        //     // swap min into place
        //     unsigned char temp = window[(tid * arraySize) + i];
...
int main(int argc, char **argv){
    ...
    // sdkSavePGM(outputfile, &origData, &width, &height);
    ...
    // int window_size = BLOCK_SIZE * BLOCK_SIZE * filterSize * filterSize * sizeof(uint);
    ...
    // // Write to file
    // // sdkSavePGM(outputfile, hOutputData, width, height);
    // #if DEBUG_MESSAGES_ON
