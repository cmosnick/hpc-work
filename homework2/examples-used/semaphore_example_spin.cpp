...
int main(int argc, char * argv[])
{
	...
	key_t key = 123457; 		// key to pass to semget(), key_t is an IPC key type defined in sys/types
	...
	if ( pid == 0 ) // Child process 
	{
		...
		// Spin some wheels
		int triesLeft = 10;
		const unsigned int spinSeconds = 1;
		while ((triesLeft--)>0 && retval != 0)
		{
		 	std::cout << "In the child (if): spinning my wheels (" << semId << ") ... " << triesLeft << std::endl; 
		 	sleep(spinSeconds);
